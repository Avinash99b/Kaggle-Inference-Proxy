...
class WorkerState:
    ...
    def __init__(self, worker_id: str) -> None:
        ...
        # Set by /worker/{id}/set-context (see request_reload_context()
        # below) for long-poll workers, which have no push channel:
        # relayed on the worker's next heartbeat/poll response, same
        # pattern as pending_kill_all. WS workers instead get an
        # immediate out-of-band "reload_model" message and don't use this
        # field.
        self.pending_reload_n_ctx: Optional[int] = None
    ...
    async def request_reload_context(self, n_ctx: int, reason: str) -> None:
        """Ask this worker to reload its model with a new n_ctx (context
        size), replacing whatever it's currently using. Delivered
        immediately over WS if connected, or queued to ride along with the
        worker's next heartbeat/poll response on long-poll -- same dual
        pattern as request_kill_all(). The worker is expected to hard-kill
        its current runner subprocess (there's no way to resize a live
        llama_context in place) and respawn it with the new n_ctx, the
        same way it already does for a cancelled/wedged job."""
        if self.transport == "ws" and self.ws is not None:
            try:
                async with self.lock:
                    await self.ws.send_text(json.dumps({"type": "reload_model", "n_ctx": n_ctx}))
                logger.info("Sent reload_model (n_ctx=%d) to worker '%s' (%s) over WS.",
                            n_ctx, self.worker_id, reason)
            except Exception as e:
                logger.warning("Failed to send reload_model to worker '%s': %s", self.worker_id, e)
        else:
            self.pending_reload_n_ctx = n_ctx
            logger.info("Queued reload_model (n_ctx=%d) for worker '%s' (%s); will be "
                        "delivered on next long-poll heartbeat.", n_ctx, self.worker_id, reason)
...
class WorkerRegistry:
    ...
    async def set_worker_context(self, worker: WorkerState, n_ctx: int,
                                  reason: str = "manual context change via dashboard/API") -> dict:
        """Tells exactly ONE worker to reload its model with a new n_ctx.
        Does not cancel or touch any in-flight/queued jobs itself -- the
        worker's own reload handling hard-kills its runner subprocess
        (which necessarily aborts whatever job that subprocess was
        running, if any) and respawns it with the new context size, the
        same teardown path used for a wedged-job kill_all reset."""
        await worker.request_reload_context(n_ctx, reason)
        return {
            "status": "ok",
            "worker_id": worker.worker_id,
            "n_ctx": n_ctx,
...
@app.post("/worker/{worker_id}/set-context")
async def set_worker_context(worker_id: str, request: Request):
    """Change the context size (n_ctx) a worker's model is loaded with.
    There's no way to resize a live llama_context in place, so this tells
    the worker to hard-kill its current runner subprocess and respawn it
    with the new n_ctx -- any job that subprocess was running is aborted
    the same way a kill_all reset would abort it. This is what the
    dashboard's per-worker context-size field + "Set" button calls.

    Body: {"n_ctx": <int>}

    Same trust boundary as /kill-all and /worker/{id}/kill: deliberately
    unauthenticated (operator-only control on the machine hosting this
    proxy, not a client-facing API surface). Put it behind a
    network-level restriction if this proxy is ever exposed somewhere
    untrusted."""
    worker = REGISTRY.get(worker_id)
    if worker is None:
        raise HTTPException(status_code=404, detail=f"unknown worker_id '{worker_id}'")
    ...
    if not isinstance(n_ctx, int) or n_ctx <= 0:
        raise HTTPException(status_code=400, detail="n_ctx must be a positive integer")
    result = await REGISTRY.set_worker_context(worker, n_ctx)
    logger.warning("Context change requested for '%s': n_ctx=%d", worker_id, n_ctx)
    return result
...
async def dashboard():
    ...
    for w in workers_sorted:
      ...
      <tr class="worker-row" onclick="showDetail('{wid}')">
        ...
        <td onclick="event.stopPropagation()">
          <input type="number" class="ctx-input" id="ctx-input-{wid}"
                 placeholder="{n_ctx_str}" min="1" step="1" />
          <button class="ctx-btn" onclick="setContext('{wid}')">Set</button>
        </td>
    ...
    <style>
        ...
        .ctx-input {{
            width: 90px; background:#0d1117; color:#c9d1d9; border:1px solid #30363d;
            border-radius:4px; padding:3px 6px; font-family: monospace; font-size:0.85em;
        }}
        .ctx-btn {{
        ...
        .ctx-btn:hover {{ background:#2ea043; }}
        .kill-all-btn {{
            background:#f85149; color:#fff; border:none; border-radius:4px;
    ...
    <table>
      <tr>
        ...
        <th>Current Model</th><th>Context Size (n_ctx)</th><th>Known Models</th><th>In-Flight</th>
    ...
    <script>
        ...
        async function setContext(workerId) {{
            const input = document.getElementById('ctx-input-' + workerId);
            const nCtx = parseInt(input.value, 10);
            if (!nCtx || nCtx <= 0) {{
                alert('Enter a positive context size (number of tokens) first.');
                return;
            }}
            if (!confirm('Reload worker "' + workerId + '" with n_ctx=' + nCtx + '? ' +
                         'This hard-kills its runner subprocess (aborting any job it is ' +
                         'currently running) and reloads the model with the new context size.')) {{
                return;
            }}
            const msg = document.getElementById('status-msg');
            msg.textContent = 'Setting context size for ' + workerId + '...';
            try {{
                const resp = await fetch('/worker/' + encodeURIComponent(workerId) + '/set-context', {{
                    method: 'POST',
                    headers: {{ 'Content-Type': 'application/json' }},
                    body: JSON.stringify({{ n_ctx: nCtx }})
                }});
                const data = await resp.json();
                if (!resp.ok) {{
                    msg.textContent = 'Error: ' + (data.detail || resp.statusText);
                ...
                msg.textContent = 'Context size for ' + workerId + ' set to ' + nCtx + '.';
                setTimeout(() => location.reload(), 800);
            }} catch (e) {{
                msg.textContent = 'Request failed: ' + e;
            }}
...
async def worker_poll(worker_id: str, timestamp: str, signature: str):
    ...
    if worker.pending_reload_n_ctx is not None:
        n_ctx = worker.pending_reload_n_ctx
        worker.pending_reload_n_ctx = None
        return {"job": None, "reload_model": {"n_ctx": n_ctx}}
...
async def worker_heartbeat(request: Request):
    ...
    if worker.pending_kill_all:
        ...
        worker.pending_kill_all = False
    if worker.pending_reload_n_ctx is not None:
        response["reload_model"] = {"n_ctx": worker.pending_reload_n_ctx}
        worker.pending_reload_n_ctx = None
    return response
