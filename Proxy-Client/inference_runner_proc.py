...
class ProxyClient:
                    ...
                    try:
                        async for raw in ws:
                            ...
                            if mtype == "job":
                                ...
                                # Dashboard/API requested a context-size
                                # change: hard-kill and respawn the runner
                                # subprocess with the new n_ctx. Same
                                # unconditional-reset shape as kill_all,
                                # just also updating the cached n_ctx
                                # first so the respawn picks it up.
                                n_ctx = msg.get("n_ctx")
                                if n_ctx:
                                    await self._handle_reload_model(int(n_ctx))
        ...
        while time.time() < end_time:
            try:
                ...
                if data.get("kill_all"):
                    await self._handle_kill_all()
                reload_req = data.get("reload_model")
                if reload_req and reload_req.get("n_ctx"):
                    await self._handle_reload_model(int(reload_req["n_ctx"]))
                job = data.get("job")
                if job is None:
                    continue
    ...
    async def _handle_reload_model(self, n_ctx: int) -> None:
        """Handles a "reload_model" command from the proxy (dashboard's
        context-size control): updates the cached n_ctx used for future
        runner spawns and force-resets the current subprocess so the
        change takes effect immediately. There's no way to resize a live
        llama_context in place, so -- same as kill_all -- this hard-kills
        whatever the runner subprocess is currently doing; any in-flight
        job on it is aborted. The next job (or the reconnect below) lazily
        respawns the subprocess via PersistentRunner.start(), which reads
        MODELS.n_ctx fresh, so setting it here first is what makes the new
        size actually stick instead of being immediately overwritten by
        the old cached value on respawn."""
        logger.warning("Received reload_model from proxy: switching to n_ctx=%d "
                        "(was %s) and force-resetting the runner subprocess.",
                        n_ctx, MODELS.n_ctx)
        ...
        MODELS.n_ctx = n_ctx
        await self.loop.run_in_executor(
            None, RUNNER.force_reset, f"reload_model received from proxy (n_ctx={n_ctx})")
